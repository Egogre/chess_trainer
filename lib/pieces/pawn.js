...
function Pawn (square, color) {
  ...
  if (color === "white") {
    ...
    this.direction = 1;
    this.type = "pawn";
    this.justMovedTwoLastTurn = false
  }
}
...
function enPassant (pawn, targetSquare) {
  ...
  return !!eligiblePawn
    ...
    && eligiblePawn.justMovedTwoLastTurn;
}

Pawn.prototype.move = function (targetSquare) {
  Piece.prototype.move.call(this, targetSquare);
  if (Math.abs(this.square.yCoordinate - targetSquare.yCoordinate) === 2) {
    this.justMovedTwoLastTurn = true;
  }
};
